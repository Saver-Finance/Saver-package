...
public struct RoomState has copy, drop, store {
...
public struct SettlementEmitted has copy, drop, store {
    round_id: u64,
    player: address,
    amount: u64,
}
...
/// NOTE: entry functions cannot return values in Sui.
entry fun create_mock_hub(
    entry_fee: u64,
    fee_bps: u64,
    escrow_total: u64,
    status: u8,
    ...
    transfer::public_transfer(hub, tx_context::sender(ctx));
}
