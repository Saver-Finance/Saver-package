module games::bomb_panic {
    ...
    enum GamePhase has copy, drop, store {
        Waiting,
        Running,
        Ended,
    }
    ...
    struct GameHubRef has copy, drop, store {
        id: ID,
    }
    ...
    struct Cashout has copy, drop, store {
        round_id: u64,
        player: address,
    ...
    struct RoundEnded has copy, drop, store {
        round_id: u64,
        ended_multiplier: u64,
        end_ts_ms: u64,
    }
    ...
    struct RoundCancelled has copy, drop, store {
        round_id: u64,
    }

    /// Create a hub with a randomness object and treasury receiver.
    public entry fun create_hub(treasury: address, rng: Random, ctx: &mut TxContext): GameHub {
        GameHub { id: object::new(ctx), treasury, rng }
    }

    /// Initialize a game state bound to a hub for a specific coin type.
    public entry fun create_game_state<T>(hub: &GameHub, ctx: &mut TxContext): GameState<T> {
        GameState {
            id: object::new(ctx),
            state: GamePhase::Waiting,
            round_id: 0,
            escrow_coin: coin::zero<T>(),
            player_entries: vector::empty(),
            total_wager: 0,
            ...
            game_config: GameConfig { max_players: MAX_PLAYERS, scale: SCALE, min_fee_bps: 0, max_fee_bps: BPS_DENOM },
            hub: GameHubRef { id: object::id(&hub.id) },
            version: 0,
        }
    }
    ...
    public entry fun start_round<T>(
        ...
        assert!(fee_bps >= game.game_config.min_fee_bps && fee_bps <= game.game_config.max_fee_bps, E_FEE_RANGE);
        assert!(object::id(&hub.id) == game.hub.id, E_HUB_MISMATCH);
        ...
        let crash_point = sample_crash_point(&hub.rng, ctx);

        // Reset round accounting.
        game.escrow_coin = escrow;
        game.player_entries = vector::empty();
        ...
        event::emit(RoundStarted { round_id: game.round_id, crash_point, fee_bps, start_ts_ms: game.round_start_ts_ms });
    }
    ...
    public entry fun cashout<T>(
        ...
        let available = if cap > game.total_payout_reserved { cap - game.total_payout_reserved } else { 0 };
        ...
        if final_payout > 0 {
            let out = coin::split(&mut game.escrow_coin, final_payout);
            transfer::transfer(out, player);
        }

        event::emit(Cashout { round_id: game.round_id, player: player_addr, cashout_multiplier: desired_multiplier, payout: final_payout });
    }
    ...
    public entry fun crash<T>(game: &mut GameState<T>, clock: &Clock) {
        ...
        event::emit(RoundEnded { round_id: game.round_id, ended_multiplier: current_mult, end_ts_ms: game.round_end_ts_ms });
    }
    ...
    public entry fun settle_round<T>(
        ...
        if available_for_fee > 0 {
            let fee_coin = coin::split(&mut game.escrow_coin, available_for_fee);
            transfer::public_transfer(fee_coin, hub.treasury);
        }

        // Remaining escrow goes to treasury as house remainder.
        let remainder_value = coin::value(&game.escrow_coin);
        if remainder_value > 0 {
            let remainder_coin = coin::split(&mut game.escrow_coin, remainder_value);
            transfer::public_transfer(remainder_coin, hub.treasury);
        }
    ...
    public entry fun force_cancel_round<T>(
        ...
        if value > 0 {
            let refund = coin::split(&mut game.escrow_coin, value);
            transfer::public_transfer(refund, hub.treasury);
        }
    ...
    fun current_multiplier<T>(game: &GameState<T>, clock: &Clock): u64 {
        ...
        let elapsed = if now_ms > game.round_start_ts_ms { now_ms - game.round_start_ts_ms } else { 0 };
    ...
    fun sample_crash_point(rng: &Random, ctx: &mut TxContext): u64 {
    ...
    fun find_player(entries: &vector<PlayerEntry>, addr: address): (Option<u64>, u64) {
        let len = vector::length(entries);
        let mut i = 0;
        while (i < len) {
            let e = vector::borrow(entries, i);
            if (e.player == addr) {
                return (option::some(i), len)
            };
            i = i + 1;
        };
        (option::none(), len)
